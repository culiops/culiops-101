...
if [[ "${SVC_STATUS}" == "ACTIVE" ]]; then
  ...
  info "Creating service '${SERVICE}' (with deployment circuit breaker + rollback)..."
  # deploymentCircuitBreaker={enable=true,rollback=true} is what makes the CI/CD
  # story safe: if the pipeline ships a bad image and the new tasks fail their
  # health check, ECS aborts the rollout and rolls back to the last good revision
  # automatically — instead of leaving the service stuck on a dead version.
